  ...
  export default {
    ...
    methods: {
      _initScroll () {
        ...
        // 动态计算ul的宽度 ,这里是Js操作，也可以直接修改ul的宽(css)，这里因为ul的宽度小于li的宽度
        //，所以导致滚动条scrollX无法滚动，这个方法是写在mounted中,
        //因为info之前在headerTop中定义加载过，这里是通过state引入Info,而不是异步请求所以不能用watch监听这里的动态，
        // 所以在调用mounted时候要判断是否有数据，没有的话在watch监听这个scrollY,
        // 如果不这样做，当你在info子路由刷新（正常进入路由时候应该在goods页面）的时候加载不到info.pics.length报错
